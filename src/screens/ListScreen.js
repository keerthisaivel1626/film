...
const ListScreen = ({route, navigation}) => {
  const filmTitle = route.params.categoryTile;
  ...
  useEffect(() => {
    ...
    fetchLocalData();
  }, [filmTitle, navigation]);
  const fetchLocalData = async () => {
    const isFilmData = JSON.parse(await AsyncStorage.getItem('filmData'));
     const categorys = isFilmData.find(
       category => category.title === filmTitle,
     );
  ...
  return (
    <View style={styles.container}>
      <ImageBackground source={bg} resizeMode="cover" style={styles.imageBg}>
        {fData ? (
          <CategoriesGridTile data={fData} color={'#ffcc66'} flag="subGrid" />
        ) : (
